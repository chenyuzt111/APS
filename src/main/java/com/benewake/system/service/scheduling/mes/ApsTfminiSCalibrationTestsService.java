package com.benewake.system.service.scheduling.mes;

import com.benewake.system.entity.ApsTfminiSCalibrationTests;
import com.baomidou.mybatisplus.extension.service.IService;
import com.benewake.system.service.ApsIntfaceDataServiceBase;

/**
* @author ASUS
* @description 针对表【aps_tfmini_s_calibration_tests】的数据库操作Service
* @createDate 2023-10-19 14:29:24
*/
public interface ApsTfminiSCalibrationTestsService extends IService<ApsTfminiSCalibrationTests> , ApsIntfaceDataServiceBase {

}
