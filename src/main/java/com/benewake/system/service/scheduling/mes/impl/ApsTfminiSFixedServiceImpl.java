...
/**
* @author DELL
* @description 针对表【aps_tfmini_s_fixed】的数据库操作Service实现
* @createDate 2023-11-27 14:35:29
*/
@Service
public class ApsTfminiSFixedServiceImpl extends ServiceImpl<ApsTfminiSFixedMapper, ApsTfminiSFixed>
    implements ApsTfminiSFixedService {



    @Autowired
    private ApsTfminiSFixedMapper apsTfminiSFixedMapper;
    @Autowired
    private MesToApsTfminiSFixed mesToApsTfminiSFixed;
    ...
    public Boolean updateDataVersions() throws Exception {
        ...
        try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
            ...
            // 解析响应内容
            // 解析响应内容
            ArrayList<ApsTfminiSFixed> apsTfminiSFixeds = null;
            if (response.getEntity() != null) {
                ...
                // 创建数据列表
                List<MesTfminiSFixed> dataList = new ArrayList<>();
                // 遍历answersArray
                for (JsonElement element : answersArray) {
                    ...
                    // 创建新的PCshaolu对象
                    MesTfminiSFixed MESFixed = new MesTfminiSFixed();
                    for (int i = 0; i < answerArray.size(); i++) {
                        ...
                        if (fieldMapping.containsKey(queTitle)) {
                            ...
                            for (int j = 0; j < valuesArray.size(); j++) {
                                ...
                                try {
                                    Field field = MESFixed.getClass().getDeclaredField(excelFieldName);
                                    field.setAccessible(true);
                                    // 如果字段是BurnQualifiedCount并且还没有设置过，设置字段值
                                    if (excelFieldName.equals("BurnQualifiedCount")) {
                                        if (id == 84950991) {
                                            field.set(MESFixed, dataValue);
                                        }
                    ...
                    // 将pcshaolu对象添加到数据列表
                    dataList.add(MESFixed);
                    apsTfminiSFixeds = new ArrayList<>();
                    for (MesTfminiSFixed mesTfminiSFixed : dataList) {
                        ApsTfminiSFixed apsTfminiSFixed = mesToApsTfminiSFixed.convert(mesTfminiSFixed,maxVersionIncr);
                        apsTfminiSFixeds.add(apsTfminiSFixed);
                    }
                }
            }
            return saveBatch(apsTfminiSFixeds);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Page selectPageList(Page page, List tableVersionList) {
        Page<ApsTfminiSFixedDto> apsTfminiSFixedDtoPage = apsTfminiSFixedMapper.selectPageList(page, tableVersionList);
        return apsTfminiSFixedDtoPage;
    }

    @Override
    public void insertVersionIncr() {
        apsTfminiSFixedMapper.insertVersionIncr();
    }
}
